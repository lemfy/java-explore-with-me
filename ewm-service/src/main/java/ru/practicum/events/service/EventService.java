...
public class EventService {
    ...
    private long getStat(Event event) {
        ...
        Object stats = statClient.getStats(LocalDateTime.of(1970, 1, 1, 0,
                0, 0), LocalDateTime.now(), uri).getBody();
    ...
    public List<EventShortDto> searchByUser(String text, Boolean paid, Boolean onlyAvailable, List<Integer> categories,
                                            LocalDateTime rangeStart, LocalDateTime rangeEnd, EventSort sort,
                                            int from, int size) {
        if (rangeStart != null && rangeEnd != null && rangeStart.isAfter(rangeEnd)) {
            throw new BadRequestException("date from must be earlier then date to");
        }

        List<Event> result = eventRepository.findAllByUser(text, onlyAvailable, paid, categories, rangeStart,
